...
(function($) {
	...
	$.jqDialog = {
		...
		prompt: function(message, callback_ok) {
			var t = this;
			modal = true;
			...
			t.create(
				$("<div>").
					append(message)
					.append( $("<span>").append( t.parts.input ) )
			...
			t.parts.input.submit( function () {
				t.close();
				if (callback_ok) {
					callback_ok( t.parts.input.val() );
					callback_ok = null;
				}
			});
		...
		//________password dialog
		password: function(message, callback_ok) {
			var t = this;
			modal = true;
			...
			t.create(
				$("<div>").
					append(message)
					.append( $("<span>").append( t.parts.passwd ) )
					.append(t.parts.bt_ok)
			);
			...
			t.parts.passwd.focus();

			t.parts.passwd.submit( function () {
				t.close();
				if (callback_ok) {
					callback_ok( t.parts.passwd.val() );
					callback_ok = null;
				}
			});
			...
			t.parts.bt_ok.unbind().click( function() {
				t.close();
				if (callback_ok) callback_ok( t.parts.passwd.val() );
			});
		},
		...
		close: function() {
			...
			$('#jqDialog_overlay').fadeOut('fast', function(){ $(this).remove(); });
			t.clearPosition();
			modal = false;
		},
		...
		buildoverlay: function() {
			$('body').append('<div id="jqDialog_overlay"></div>');

			// Set the proper height
			$('#jqDialog_overlay').css('height', $(document).height());
			$('#jqDialog_overlay').css('width', $(document).width());

			// Fade it in
			$('#jqDialog_overlay').css('opacity',0).fadeTo(500, 0.2);
		},
		...
		init: function() {
			...
			t.parts.passwd = $("<input type=\"password\">").attr({ id: t.ids.passwd });
			t.parts.input = $("<input type=\"text\">").attr({ id: t.ids.input });
