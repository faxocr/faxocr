class FaxocrController < ApplicationController
  ...
  def group_select
    if request.post?
      ...
      if role_mapping
        ...
        flash.now[:notice] = "Invalid group id."
        return
      end
    end
  end
  ...
  def logout
    ...
    if session[:masq_from_user]
      ...
      flash[:notice] = "Logged out from #{@current_user.full_name}."
  ...
  def masquerade
    if request.post?
      ...
      unless masq_to_user
        flash[:notice] = "User id:#{params[:id]} does not exist"
        redirect_to :action => 'index'
        return
      end
      if session[:masq_from_user]
        flash[:notice] = "You are already masquerading."
        redirect_to :action => 'index'
        return
      end
